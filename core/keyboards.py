...
def get_block_menu_kb(order_id, history_map, lang="uz_latin"):
    ...
    if len(history_map) >= 4:
        builder.row(InlineKeyboardButton(text=_("tr_start", lang), callback_data=f"tr_start_{order_id}"))
    
    return builder.as_markup()

def get_block_selection_kb(letter, order_id, lang="uz_latin"):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=_("ortdi", lang), callback_data=f"block_act_{letter}_ortdi_{order_id}")],
        [InlineKeyboardButton(text=_("ortmadi", lang), callback_data=f"block_act_{letter}_ortmadi_{order_id}")],
        [InlineKeyboardButton(text=_("back", lang), callback_data=f"back_to_blocks_{order_id}")]
    ])

def get_transit_kb(order_id, lang="uz_latin"):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=_("oldi", lang), callback_data=f"tr_oldi_1_{order_id}")],
        [InlineKeyboardButton(text=_("olmadim", lang), callback_data=f"tr_olmadim_{order_id}")]
    ])
...
def get_admin_panel_kb(lang: str = "uz_latin"):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=_("adm_cars_status", lang), callback_data="adm_cars_status")],
        [InlineKeyboardButton(text=_("adm_active", lang), callback_data="adm_active")],
        [InlineKeyboardButton(text=_("adm_hist_car", lang), callback_data="adm_hist_car")],
        [InlineKeyboardButton(text=_("adm_hist_drv", lang), callback_data="adm_hist_drv")],
        [InlineKeyboardButton(text=_("adm_hist_all", lang), callback_data="adm_hist_all")],
        [InlineKeyboardButton(text=_("adm_refresh", lang), callback_data="adm_refresh")],
        [InlineKeyboardButton(text=_("adm_close", lang), callback_data="adm_close")]
    ])
